...
interface ShareDrawerContentProps {
  prayer: PrayerData;
  reminder: PrayerData; // Added reminder prop
  onClose: () => void;
}

export default function ShareDrawerContent({ prayer, reminder, onClose }: ShareDrawerContentProps) {
  const colorScheme = useColorScheme();
  const viewShotRef = useRef<ViewShot>(null);
  const [prayerFormula, setPrayerFormula] = useState<PrayerFormula | null>(null);

  // Détecter automatiquement le type d'élément
  const isReminder = prayer && prayer.hasOwnProperty('title');
  const isPrayer = prayer && prayer.hasOwnProperty('name');
  const actualPrayer = isPrayer ? prayer : undefined;
  const actualReminder = isReminder ? prayer : reminder;
  ...
  }, [getRandomPrayerFormula, actualPrayer]);

  // Déterminer le type de contenu et créer le texte de partage approprié
  const getShareText = () => {
    if (actualPrayer) {
      return `Prière pour le défunt\n\n${actualPrayer.name}\n${actualPrayer.age} ans • ${formatDate(actualPrayer.deathDate)}\n${actualPrayer.location}`;
    } else if (actualReminder) {
      return `Rappel: ${actualReminder.title}\n\n${actualReminder.description}\n\n${actualReminder.arabic}\n${actualReminder.transliteration}\n${actualReminder.translation}`;
    }
    return '';
  };
        ...
        <View style={[
          ...
          <View style={styles.deceasedInfo}>
            <Text style={[
              ...
              {actualPrayer ? actualPrayer.name : actualReminder?.title}
            </Text>
            ...
            <View style={styles.detailsRow}>
              ...
              <Text style={[
                ...
                {actualPrayer ? `${actualPrayer.age} ans • ${formatDate(actualPrayer.deathDate)}` : actualReminder?.description}
              </Text>
            </View>
            ...
            <View style={styles.detailsRow}>
              ...
              <Text style={[
                ...
                {actualPrayer ? actualPrayer.location : actualReminder?.arabic}
              </Text>
            </View>
          </View>

          {/* Formule de prière si disponible */}
          {actualPrayer && prayerFormula && (
            <>
