...
export class LikeService {
  ...
  static async getPrayerLikes(prayerId: string): Promise<{
    success: boolean;
    data?: LikeData[];
    count?: number;
    error?: string;
    ...
    try {
      const q = query(collection(db, LIKES_COLLECTION), where('prayerId', '==', prayerId));
  ...
  static async getUserLikes(
    ...
    try {
      ...
      if (userId) {
        q = query(collection(db, LIKES_COLLECTION), where('userId', '==', userId));
      } else if (deviceId) {
        q = query(collection(db, LIKES_COLLECTION), where('deviceId', '==', deviceId));
      } else {
        return { success: false, error: 'UserId ou deviceId requis' };
      }
